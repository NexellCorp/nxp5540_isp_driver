...
static int register_v4l2(struct btree_usb *dev)
{
	...
	btree_video_setUSBHandle(dev);
	return 0;
}

static void unregister_v4l2(struct btree_usb *dev)
{
	dev_info(&dev->udev->dev, "%s", __func__);
	if (dev->vdev) {
		dev_info(&dev->udev->dev, "%p\n", dev->vdev);
...
static void btree_read_bulk_callback(struct urb *urb)
{
	...
	if (!available)
		rv = btree_recv_frame(dev, dev->bulk_in_bufsize);
	if (copy_to_user(source + dev->bulk_in_copied,
					dev->bulk_in_buffer,
					chunk))
		rv = -EFAULT;
	else
	...
	if (dev->bulk_in_copied < dev->bulk_in_bufsize)
		rv = btree_recv_frame(dev, dev->bulk_in_bufsize - dev->bulk_in_copied);
}

static int btree_recv_frame_dmabuf
	(struct btree_usb *dev, dma_addr_t buffer, int count)
{
	int rv;
...
int btree_recv_frame
	(struct btree_usb *dev, int count)
{
	int rv;
	 ...
	 int value, void *buf, int len)
{
	...
	retval = usb_control_msg(dev->udev,
			...
			request, USB_TYPE_VENDOR | dir | USB_RECIP_DEVICE, value, 0x0, buf, len, BTREE_USB_CTL_TIMEOUT*5);
...
(struct btree_usb *udev, dma_addr_t buffer, size_t count)
{
	struct btree_usb *dev;
	int rv;
	bool ongoing_io;
...
retry:
	...
	//dev_info(&dev->interface->dev, "ongoind_io is %d \n",
	//		ongoing_io);
	if (ongoing_io) {
		rv = wait_event_interruptible(dev->bulk_in_wait, (!dev->ongoing_read));
		if (rv < 0) {
			dev_err(&dev->interface->dev, "wait interrupt fail \n",
			ongoing_io);
			goto exit;
		}
	}
	...
	rv = dev->errors;
	if (rv < 0) {
		dev_err(&dev->interface->dev, "error - %d \n",
				rv);
		dev->errors = 0;
		rv = (rv == -EPIPE) ? rv : -EIO;
		goto exit;
	...
	//dev_info(&dev->interface->dev, "bulk in filled is %d \n",
	//		dev->bulk_in_filled);

	if (dev->bulk_in_filled) {
		size_t available = dev->bulk_in_filled - dev->bulk_in_copied;
		if (!available) {
			rv = btree_recv_frame_dmabuf(dev, buffer + dev->bulk_in_copied,
		...
		rv = btree_recv_frame_dmabuf(dev, buffer, count);
		if (rv < 0)
			goto exit;
		else
			goto retry;
